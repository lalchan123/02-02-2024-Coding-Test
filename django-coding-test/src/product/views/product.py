...
def ProductListView(request):
    ...
    # Product Variant Size, Color, Style
    sizeItem = []
    colorItem = []
    styleItem = []
    ...
    #  pagination with two products in per page
...
def SearchView(request):
    ...
    if request.method == 'POST':
        ...
        try:
            # search by title, variant, price_from, price_to, date and search result store in queryset
...
def CreateProductApi(request):
    try:
        ...
        #Product create and the data will be saved in Product model
        product = Product.objects.create(title=productName, sku=productSku, description=productDescription)

        #ProductImage create and the data will be saved in ProductImage model
        ProductImage.objects.create(product=product, file_path=file_path)

        # ProductVariant create and the data will be saved in productVariant model by Size, Color, Style
        ...
        # ProductVariantPrice create and the data will be saved in ProductVariantPrice model 
        for pvp in productVariantPrice:
            alength = len(pvp['title'].split("/"))-1
            count = 0

            # find out the product variant
            for pl in range(alength):
                # when pl==0 then product_variant is product_variant_one and count is 1
                if pl==0:
                    count+=1
                    product_variant_one=ProductVariant.objects.get(variant_title=pvp['title'].split("/")[pl],product=product)
                
                # when pl==1 then product_variant is product_variant_two and count is 2
                if pl==1:
                    count+=1
                    product_variant_two=ProductVariant.objects.get(variant_title=pvp['title'].split("/")[pl],product=product)
                
                # when pl==2 then product_variant is product_variant_three and and count is 3
                if pl==2:
                    count+=1
                    product_variant_three=ProductVariant.objects.get(variant_title=pvp['title'].split("/")[pl],product=product)

            # when count==1 then product_variant is only one product_variant_one and the data will be saved in ProductVariantPrice model    
            if count==1:
            ...
            # when count==2 then product_variant is product_variant_one, product_variant_two and the data will be saved in ProductVariantPrice model      
            if count==2:
                ProductVariantPrice.objects.create(
                    product_variant_one=product_variant_one,
            ...
            # when count==3 then product_variant is product_variant_one, product_variant_two, product_variant_three and the data will be saved in ProductVariantPrice model       
            if count==3:
                ProductVariantPrice.objects.create(
                    product_variant_one=product_variant_one,
                    product_variant_two=product_variant_two,
...
def GetProductUpdateApi(request, id):
    try:
        ...
        product = Product.objects.get(id=id)
         ...
         # filter the ProductImage, ProductVariant, ProductVariantPrice data by product
...
def ProductUpdateApi(request, id):
    try:
        ...
        # ProductImage Updated by product
